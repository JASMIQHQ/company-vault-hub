...
export function expiryState(document: CompanyDocument): "expired" | "expiring" | "valid" | null {
...
export interface ReadinessSummary {
  ...
  /** Average of the backend-provided compliance_percentage across analyzed tenders. */
...
/** Derives today's actions strictly from real document, tender and requirement rows. */
export function buildMissions(
  ...
  for (const doc of documents) {
    ...
    if (state === "expired") {
      missions.push({
        ...
        urgency: "high",
        title: `Replace ${doc.document_name}`,
        detail: "This document is expired and can disqualify a submission.",
        deadline: doc.expiry_date,
        action: { label: "Open Company Vault", to: "/vault" },
      });
      ...
      missions.push({
        ...
        detail: "Expiring within 30 days — renew before your next submission.",
        deadline: doc.expiry_date,
        action: { label: "Open Company Vault", to: "/vault" },
      });
    }
  ...
  for (const tender of tenders) {
    ...
    if (status === "failed") {
      missions.push({
        id: `tender-failed-${tender.id}`,
        urgency: "high",
        title: "Retry tender analysis",
        detail: `${label} — analysis did not complete.`,
        deadline: tender.submission_deadline,
        action: { label: "Open Tender Command", to: "/tenders" },
      });
    } else if (status !== "analyzed" && status !== "processing") {
      missions.push({
        id: `tender-analyze-${tender.id}`,
        urgency: "medium",
        title: "Analyze tender",
        detail: `${label} — requirements have not been extracted yet.`,
        deadline: tender.submission_deadline,
        action: { label: "Open Tender Command", to: "/tenders" },
      });
    }
    ...
    const review = rows.filter(
      (row) => row.status === "manual_review" || row.status === "pending",
    ).length;

    if (missing + expiredReq > 0) {
      missions.push({
        id: `tender-gaps-${tender.id}`,
        urgency: "high",
        title: "Close tender requirement gaps",
        detail: `${label} — ${missing} missing and ${expiredReq} expired requirement${
          missing + expiredReq === 1 ? "" : "s"
        }.`,
        deadline: tender.submission_deadline,
        action: { label: "Review Tender", to: "/tenders" },
      });
    }

    if (review > 0) {
      missions.push({
        id: `tender-review-${tender.id}`,
        urgency: "medium",
        title: "Review tender requirements",
        detail: `${label} — ${review} requirement${review === 1 ? "" : "s"} need human verification.`,
        deadline: tender.submission_deadline,
        action: { label: "Review Tender", to: "/tenders" },
      });
    }
    ...
    if (specific.length > 0) {
      missions.push({
        id: `tender-specific-${tender.id}`,
        urgency: "medium",
        title: "Prepare tender-specific documents",
        detail: `${label} requires: ${specific.join(", ")}. These must be obtained for this tender.`,
        deadline: tender.submission_deadline,
        action: { label: "Open Tender Command", to: "/tenders" },
      });
    }
...
export function buildDeadlines(tenders: TenderListItem[]): DeadlineItem[] {
  return tenders
    .filter((tender) => Boolean(tender.submission_deadline))
    .map((tender) => {
