...
/**
 * Extracts known machine failure reasons from embedded diagnostics while
 * stripping debug metadata and internal UUIDs from customer-facing copy.
 */
export function parseRequirementExplanation(
  rawExplanation: string | null | undefined,
): ExtractedDiagnostics {
  if (!rawExplanation) {
    return { cleanExplanation: "", failureReason: null };
  }
