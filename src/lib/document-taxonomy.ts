...
export type FederalTenderDocumentType =
  (typeof FEDERAL_TENDER_DOCUMENT_TYPES)[number];

export const DOCUMENT_TYPE_ALIASES: Record<
  string,
  FederalTenderDocumentType
> = {
  CAC: "CAC_CERTIFICATE",
  ...
  "NATIONAL PENSION COMMISSION COMPLIANCE CERTIFICATE":
    "PENCOM_CERTIFICATE",
  ITF: "ITF_CERTIFICATE",
  "INDUSTRIAL TRAINING FUND COMPLIANCE CERTIFICATE": "ITF_CERTIFICATE",
  NSITF: "NSITF_CERTIFICATE",
  "NIGERIA SOCIAL INSURANCE TRUST FUND COMPLIANCE CERTIFICATE":
    "NSITF_CERTIFICATE",
  BPP: "BPP_CERTIFICATE",
  "BUREAU OF PUBLIC PROCUREMENT CERTIFICATE OF REGISTRATION":
    "BPP_CERTIFICATE",
  CPN: "CPN_CERTIFICATE",
  "COMPUTER PROFESSIONALS OF NIGERIA": "CPN_CERTIFICATE",
  NEMSA: "NEMSA_CERTIFICATE",
  "NIGERIAN ELECTRICITY MANAGEMENT SERVICES AGENCY CERTIFICATE":
    "NEMSA_CERTIFICATE",
};

export function normalizeDocumentType(
  value: string | null | undefined,
): FederalTenderDocumentType | null {
  if (!value) return null;
  const normalized = value.trim().toUpperCase().replace(/\s+/g, " ");
  if (
    (FEDERAL_TENDER_DOCUMENT_TYPES as readonly string[]).includes(normalized)
  ) {
