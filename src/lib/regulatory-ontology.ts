...
/**
 * Procurement-facing regulatory identity map.
 *
 * This is intentionally about identity, not blanket validity. A historical
 * issuer can identify the document class without proving that the document is
 * currently acceptable for a tender.
 */
...
export function resolveRegulatoryConcept(value: string | null | undefined) {
  ...
  for (const concept of Object.values(REGULATORY_ONTOLOGY)) {
    if (concept.aliases.some((alias) => normalized.includes(normalizeRegulatoryText(alias)))) return concept;
  }
  return null;
}
