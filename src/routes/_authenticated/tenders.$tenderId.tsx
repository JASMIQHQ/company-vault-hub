...
function TenderWorkspacePage() {
  ...
  const isReading = status === "processing" || analysisSubmitting || analyze.isPending;
  // Explicit priority: processing wins; otherwise saved requirements decide Read vs Re-read.
  const readActionLabel = isReading ? "Reading tender..." : hasSavedRequirements ? "Re-read tender" : "Read Tender";
  const displayRequirements = isReading ? [] : requirements;
  ...
  const baseReadiness = deriveTenderReadiness(displayRequirements);
  ...
  const analyzeNow = async () => { if (isReading) return; setAnalysisSubmitting(true); try { await analyze.mutateAsync(tender.id); await Promise.all([tenderQuery.refetch(), requirementsQuery.refetch(), documentsQuery.refetch()]); toast.success(hasSavedRequirements ? "Tender re-read and Vault matching refreshed" : "Tender read and Vault matching completed"); } catch (error) { toast.error(error instanceof Error ? error.message : "Tender reading failed"); } finally { setAnalysisSubmitting(false); } };
  ...
  return <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
    ...
    <header className="glass-panel overflow-hidden rounded-2xl p-5 sm:p-7">
      ...
      {status === "failed" && tender.analysis_error ? <div className="mt-3 rounded-xl border border-destructive/25 bg-destructive/5 p-3 text-sm text-destructive"><strong>Tender reading error:</strong> {tender.analysis_error}</div> : null}
      {!isReading && !hasSavedRequirements && (status === "analyzed" || status === "requires_review") ? <div className="mt-3 rounded-xl border border-warning/25 bg-warning-soft/30 p-3 text-sm text-warning"><strong>No requirements were saved.</strong> Re-read the tender before running or relying on Vault matching.</div> : null}
    ...
    <TenderActionPlan requirements={displayRequirements} documents={documentsQuery.data ?? []} lots={lotsQuery.data ?? []} selectedLotId={selectedLotId} organizationId={tender.organization_id} tenderId={tender.id} />

    <section className="glass-panel mt-5 rounded-2xl p-4 sm:p-5"><div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm"><span className="font-medium">{displayRequirements.length} Requirements</span><span className="text-success">✓ {counts.matched} Matched</span><span className="text-warning">⚠ {counts.manualReview} Manual Review</span><span className="text-destructive">✕ {counts.missing} Missing</span><span className="text-warning">⏱ {counts.expired} Expired</span></div></section>
