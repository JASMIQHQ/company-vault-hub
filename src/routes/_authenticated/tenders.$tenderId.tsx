...
function TenderWorkspacePage() {
  ...
  const hasReadTender = status === "analyzed" || status === "requires_review";
  const readActionLabel = hasReadTender ? "Re-read tender" : "Read Tender";
  ...
  const analyzeNow = async () => { if (analysisSubmitting || analyze.isPending || status === "processing") return; setAnalysisSubmitting(true); try { await analyze.mutateAsync(tender.id); await Promise.all([tenderQuery.refetch(), requirementsQuery.refetch(), documentsQuery.refetch()]); toast.success(hasReadTender ? "Tender re-read and Vault matching refreshed" : "Tender read and Vault matching completed"); } catch (error) { toast.error(error instanceof Error ? error.message : "Tender reading failed"); } finally { setAnalysisSubmitting(false); } };
  ...
  return <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
    ...
    <header className="glass-panel overflow-hidden rounded-2xl p-5 sm:p-7">
      ...
      {hasReadTender && !hasSavedRequirements ? <div className="mt-3 rounded-xl border border-warning/25 bg-warning-soft/30 p-3 text-sm text-warning"><strong>No requirements were saved.</strong> Re-read the tender before running or relying on Vault matching.</div> : null}
      <div className="mt-4 rounded-xl border border-primary/15 bg-primary/5 p-3 text-sm text-muted-foreground"><strong className="text-foreground">{hasReadTender ? "Tender requirements are stored." : "Read the tender once."}</strong> JASMIQ uses AI to interpret the uploaded tender and save its requirement checklist. After that, Vault matching is deterministic and does not use AI; re-reading is only for an explicit tender refresh.</div>
    </header>
