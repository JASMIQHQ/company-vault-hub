...
function TenderWorkspacePage() {
  ...
  const analyzeNow = async () => { if (analysisSubmitting || analyze.isPending || status === "processing") return; setAnalysisSubmitting(true); try { await analyze.mutateAsync(tender.id); await Promise.all([tenderQuery.refetch(), requirementsQuery.refetch(), documentsQuery.refetch()]); toast.success("Tender analysis refreshed"); } catch (error) { toast.error(error instanceof Error ? error.message : "Tender analysis failed"); } finally { setAnalysisSubmitting(false); } };
