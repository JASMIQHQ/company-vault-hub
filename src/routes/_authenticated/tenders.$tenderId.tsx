...
function statusMessage(status: AnalysisStatus) {
  switch (status) {
    ...
    case "analyzed": return "Showing the requirements and evidence state currently stored by the analysis pipeline.";
...
function TenderWorkspacePage() {
  ...
  const matchedCount = satisfiedCount;
  ...
  return <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
    ...
    <section className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
      <Metric label="Satisfied" value={String(satisfiedCount)} />
      <Metric label="Needs review" value={String(reviewCount)} />
      <Metric label="Missing" value={String(missingCount)} />
      <Metric label="Expired" value={String(expiredCount)} />
      <Metric label="Readiness" value={`${readinessScore}%`} />
    </section>

    <section className="glass-panel mt-5 rounded-2xl p-5 sm:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div><h2 className="text-lg font-semibold">Bid Readiness</h2><p className="mt-1 text-sm text-muted-foreground">Requirement-level readiness based only on evidence linked to the tender's selected company.</p></div>
        <Badge variant="outline" className="w-fit rounded-full">{requirements.length ? `${readinessScore}% ready` : "Awaiting analysis"}</Badge>
      </div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><ReadinessItem label="Satisfied" value={satisfiedCount} /><ReadinessItem label="Needs review" value={reviewCount} /><ReadinessItem label="Missing" value={missingCount} /><ReadinessItem label="Expired" value={expiredCount} /></div>
    </section>
