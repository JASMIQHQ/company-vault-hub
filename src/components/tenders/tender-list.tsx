...
function RowActions({ tender }: { tender: TenderListItem }) {
  ...
  return <div className="flex justify-end gap-1"><Button variant="ghost" size="sm" className="rounded-lg" onClick={() => open("preview")} disabled={disabled} aria-label={`Preview ${tender.title}`}>{busy === "preview" ? <Loader2 className="size-4 animate-spin" /> : <Eye className="size-4" />}<span className="ml-1.5 hidden sm:inline">Preview</span></Button><Button variant="ghost" size="sm" className="rounded-lg" onClick={() => open("download")} disabled={disabled} aria-label={`Download ${tender.title}`}>{busy === "download" ? <Loader2 className="size-4 animate-spin" /> : <Download className="size-4" />}<span className="ml-1.5 hidden sm:inline">Download</span></Button></div>;
}
...
function AnalyzeButton({ tender, onAnalyze }: { tender: TenderListItem; onAnalyze: (tenderId: string) => Promise<void> }) {
  ...
  return <Button variant="outline" size="sm" className="rounded-lg" disabled={processing} onClick={async () => { try { await analyze.mutateAsync(tender.id); toast.success("Analysis started"); } catch (error) { toast.error(error instanceof Error ? error.message : "Analysis failed"); } await onAnalyze(tender.id); }}>{processing ? <Loader2 className="size-4 animate-spin" /> : <Sparkles className="size-4" />}<span className="ml-1.5">{label}</span></Button>;
}
...
export function TenderList({ session, tenders, isLoading, error, onRetry }: TenderListProps) {
  ...
  return <div><div className="flex flex-col gap-3 border-b border-border/50 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="relative w-full sm:max-w-sm"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search tenders or procuring entity" className="rounded-xl pl-9 pr-9" aria-label="Search tenders" />{search ? <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1/2 size-7 -translate-y-1/2 rounded-lg" onClick={() => setSearch("")} aria-label="Clear tender search"><X className="size-4" /></Button> : null}</div><div className="flex flex-wrap gap-2"><select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)} className="h-9 rounded-xl border border-border/60 bg-background px-3 text-xs"><option value="all">All statuses</option><option value="pending">Pending</option><option value="processing">Processing</option><option value="analyzed">Analyzed</option><option value="requires_review">Requires review</option><option value="failed">Failed</option></select><select value={deadlineFilter} onChange={(event) => setDeadlineFilter(event.target.value)} className="h-9 rounded-xl border border-border/60 bg-background px-3 text-xs"><option value="all">All deadlines</option><option value="Overdue">Overdue</option><option value="Critical">Critical</option><option value="Upcoming">Upcoming</option><option value="Normal">Normal</option><option value="none">No deadline</option></select></div></div>
    {filteredTenders.length === 0 ? <div className="p-14 text-center"><p className="text-sm font-medium">No tenders match your filters.</p><p className="mt-1 text-sm text-muted-foreground">Try a different search or filter.</p></div> : <div className="overflow-x-auto"><Table><TableHeader><TableRow className="hover:bg-transparent"><TableHead>Tender name</TableHead><TableHead className="hidden sm:table-cell">Deadline</TableHead><TableHead className="hidden md:table-cell">Uploaded</TableHead><TableHead className="hidden sm:table-cell">Status</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader><TableBody>{filteredTenders.map((tender) => { const status = tender.analysis_status ?? "pending"; const isOpen = expanded === tender.id; const showDetails = status !== "pending"; const deadline = deadlineState(tender.submission_deadline); return <Fragment key={tender.id}><TableRow className="transition-colors"><TableCell className="font-medium"><div className="flex items-center gap-1.5">{showDetails ? <button type="button" onClick={() => setExpanded(isOpen ? null : tender.id)} className="rounded-md p-0.5 text-muted-foreground hover:text-foreground" aria-label={isOpen ? "Hide analysis" : "Show analysis"}>{isOpen ? <ChevronDown className="size-4" /> : <ChevronRight className="size-4" />}</button> : <span className="inline-block size-5" />}<Link to="/tenders/$tenderId" params={{ tenderId: tender.id }} className="truncate hover:underline">{tender.title}</Link></div></TableCell><TableCell className="hidden sm:table-cell"><div className="text-sm">{tender.submission_deadline ? formatDate(tender.submission_deadline) : "—"}</div>{deadline ? <div className={`text-xs font-medium ${deadline.className}`}>{deadline.label}</div> : <div className="text-xs text-muted-foreground">No deadline recorded</div>}</TableCell><TableCell className="hidden md:table-cell text-muted-foreground">{formatDate(tender.created_at)}</TableCell><TableCell className="hidden sm:table-cell"><StatusBadge status={status as never} /></TableCell><TableCell className="text-right"><div className="flex flex-wrap items-center justify-end gap-1">{status === "analyzed" || status === "requires_review" ? <TenderBidStructure tenderId={tender.id} compact /> : null}<AnalyzeButton tender={tender} onAnalyze={async (id) => setExpanded(id)} /><RowActions tender={tender} /></div></TableCell></TableRow>{isOpen ? <TableRow className="hover:bg-transparent"><TableCell colSpan={5} className="bg-muted/20"><div className="space-y-4 py-2"><div className="grid gap-3 sm:grid-cols-2"><div><p className="text-xs uppercase tracking-wide text-muted-foreground">Procuring Entity</p><p className="text-sm font-medium">{tender.procuring_entity ?? "—"}</p></div><div><p className="text-xs uppercase tracking-wide text-muted-foreground">Submission Deadline</p><p className="text-sm font-medium">{tender.submission_deadline ? formatDate(tender.submission_deadline) : "—"}</p></div></div><div><p className="mb-2 text-sm font-semibold tracking-tight">Requirement Checklist</p><RequirementChecklist session={session} tender={tender} /></div><TenderSpecificRequirements tender={tender} />{status === "failed" ? <AnalyzeButton tender={tender} onAnalyze={async (id) => setExpanded(id)} /> : null}</div></TableCell></TableRow> : null}</Fragment>; })}</TableBody></Table></div>}
  </div>;
}
