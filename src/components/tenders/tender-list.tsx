    ...
    <div className="min-w-0 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs sm:grid-cols-3">
        <div className="rounded-xl border border-border/60 bg-background/40 p-2.5">
          ...
          <p className="mt-0.5 font-medium">{tender.submission_deadline ? formatDate(tender.submission_deadline) : "—"}</p>
          <p className={`mt-0.5 font-medium ${deadline?.className ?? "text-muted-foreground"}`}>{deadline?.label ?? "No deadline"}</p>
        </div>
        <div className="rounded-xl border border-border/60 bg-background/40 p-2.5">
          <p className="text-muted-foreground">Status</p>
          <div className="mt-1"><StatusBadge status={status as never} /></div>
        </div>
        <div className="col-span-2 rounded-xl border border-border/60 bg-background/40 p-2.5 sm:col-span-1">
          <p className="text-muted-foreground">Uploaded</p>
          <p className="mt-0.5 font-medium">{formatDate(tender.created_at)}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <article className="min-w-0 rounded-2xl border border-border/60 bg-background/40 p-3 shadow-sm sm:p-4">
      ...
      <div className="mt-3 flex min-w-0 flex-wrap gap-2">
        {status === "analyzed" || status === "requires_review" ? <TenderBidStructure tenderId={tender.id} compact /> : null}
        <AnalyzeButton tender={tender} onAnalyze={onAnalyze} />
      </div>
      <div className="mt-2 flex min-w-0 flex-wrap justify-end gap-1 border-t border-border/50 pt-2">
        <RowActions tender={tender} />
      </div>
      {showDetails ? (
        <button type="button" onClick={onToggle} className="mt-2 flex min-h-9 w-full items-center justify-between rounded-xl border border-border/50 bg-muted/20 px-3 text-left text-xs font-medium hover:bg-muted/40" aria-expanded={expanded}>
          <span>{expanded ? "Hide analysis details" : "View analysis details"}</span>
    ...
    <div className="min-w-0">
      <div className="flex flex-col gap-3 border-b border-border/50 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative w-full sm:max-w-sm">
          ...
          <Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search tenders or procuring entity" className="rounded-xl pl-9 pr-9" aria-label="Search tenders" />
          {search ? <Button type="button" variant="ghost" size="icon" className="absolute right-1 top-1/2 size-7 -translate-y-1/2 rounded-lg" onClick={() => setSearch("")} aria-label="Clear tender search"><X className="size-4" /></Button> : null}
        </div>
      ...
      {filteredTenders.length === 0 ? <div className="p-14 text-center"><p className="text-sm font-medium">No tenders match your filters.</p><p className="mt-1 text-sm text-muted-foreground">Try a different search or filter.</p></div> : (
        <>
          <div className="space-y-3 p-3 md:hidden">
            {filteredTenders.map((tender) => <MobileTenderCard key={tender.id} session={session} tender={tender} expanded={expanded === tender.id} onToggle={() => setExpanded(expanded === tender.id ? null : tender.id)} onAnalyze={async (id) => setExpanded(id)} />)}
          </div>
          <div className="hidden overflow-x-auto md:block">
            <Table>
              <TableHeader><TableRow className="hover:bg-transparent"><TableHead>Tender name</TableHead><TableHead>Deadline</TableHead><TableHead>Uploaded</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
              <TableBody>
                {filteredTenders.map((tender) => {
                  ...
                  return <Fragment key={tender.id}>
                    <TableRow className="transition-colors">
                      <TableCell className="font-medium"><div className="flex min-w-0 items-center gap-1.5">{showDetails ? <button type="button" onClick={() => setExpanded(isOpen ? null : tender.id)} className="rounded-md p-0.5 text-muted-foreground hover:text-foreground" aria-label={isOpen ? "Hide analysis" : "Show analysis"}>{isOpen ? <ChevronDown className="size-4" /> : <ChevronRight className="size-4" />}</button> : <span className="inline-block size-5" />}<Link to="/tenders/$tenderId" params={{ tenderId: tender.id }} className="truncate hover:underline">{tender.title}</Link></div></TableCell>
                      <TableCell><div className="text-sm">{tender.submission_deadline ? formatDate(tender.submission_deadline) : "—"}</div>{deadline ? <div className={`text-xs font-medium ${deadline.className}`}>{deadline.label}</div> : <div className="text-xs text-muted-foreground">No deadline recorded</div>}</TableCell>
                      <TableCell className="text-muted-foreground">{formatDate(tender.created_at)}</TableCell>
                      <TableCell><StatusBadge status={status as never} /></TableCell>
                      <TableCell className="text-right"><div className="flex min-w-0 flex-wrap items-center justify-end gap-1">{status === "analyzed" || status === "requires_review" ? <TenderBidStructure tenderId={tender.id} compact /> : null}<AnalyzeButton tender={tender} onAnalyze={async (id) => setExpanded(id)} /><RowActions tender={tender} /></div></TableCell>
                    </TableRow>
