...
import { createTenderLot, listTenderLots } from "@/lib/tender-lots";
...
export function TenderLotManager({ tenderId, organizationId, companyId, selectedLotId, onSelectLot }: TenderLotManagerProps) {
  ...
  const save = async () => {
    ...
    } catch (error) { toast.error(error instanceof Error ? error.message : "Could not create the lot"); }
  };
  return <section className="glass-panel mt-5 rounded-2xl p-5 sm:p-6">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><div className="flex items-center gap-2"><Layers3 className="size-5 text-primary" /><h2 className="text-lg font-semibold">Tender lots</h2></div><p className="mt-1 text-sm text-muted-foreground">Scope requirements and readiness to the exact lot your company is bidding for.</p></div><Button className="rounded-xl" onClick={() => setOpen(true)}><Plus className="mr-2 size-4" />Add lot</Button></div>
    <div className="mt-4 flex flex-wrap gap-2"><Button type="button" variant={selectedLotId === null ? "default" : "outline"} className="rounded-full" size="sm" onClick={() => onSelectLot(null)}>Whole tender</Button>{lots.map((lot) => <Button key={lot.id} type="button" variant={selectedLotId === lot.id ? "default" : "outline"} className="rounded-full" size="sm" onClick={() => onSelectLot(lot.id)}>Lot {lot.lot_number}</Button>)}</div>
