      ...
      <DialogContent className="jasmiq-glass-dialog glass-panel sm:max-w-xl">
        <div className="jasmiq-glass-stage -m-6 p-6">
          ...
          <div className="mt-6 space-y-4">
            ...
            {lots.length > 0 ? <div className="grid gap-3 sm:grid-cols-2">{lots.map((lot: TenderLot, index) => <button type="button" key={lot.id} onClick={() => { onSelectLot?.(lot.id); setOpen(false); }} style={{ "--scope-delay": `${index * 70}ms` } as React.CSSProperties} className={`jasmiq-scope-card group w-full rounded-2xl border p-4 text-left ${selectedLotId === lot.id ? "border-primary/45 bg-primary/5 ring-1 ring-primary/15" : "border-border/60"}`}><div className="relative z-10 flex items-start gap-3"><div className="jasmiq-scope-icon flex size-10 shrink-0 items-center justify-center rounded-xl border border-primary/15 bg-primary/10 text-primary"><Layers3 className="size-4" /></div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><Badge variant="outline" className="rounded-full bg-background/20">{lot.lot_number}</Badge>{selectedLotId === lot.id ? <span className="text-xs font-semibold text-primary">Selected</span> : <span className="text-xs text-muted-foreground transition-colors group-hover:text-foreground">Select</span>}</div><p className="mt-3 text-sm font-semibold tracking-tight">{lot.lot_title}</p>{lot.description ? <p className="mt-1 text-xs leading-5 text-muted-foreground">{lot.description}</p> : null}</div></div></button>)}</div> : null}
          </div>
        </div>
      </DialogContent>
    </Dialog>
