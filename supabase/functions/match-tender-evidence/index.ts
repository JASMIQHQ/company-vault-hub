...
const ORIGINS = new Set(["https://company-vault-hub.netlify.app"]);
...
const SPECIAL:[RegExp,string][]=[
 [/audited (financial )?statements?|audited accounts/i,"AUDITED_ACCOUNTS"],
 [/bank reference/i,"BANK_REFERENCE"],
 [/company profile/i,"COMPANY_PROFILE"],
 [/sworn affidavit/i,"SWORN_AFFIDAVIT"],
 [/nitda.*(registration|contractor)/i,"NITDA_REGISTRATION"],
 [/ogisp/i,"OGISP_REGISTRATION"],
 [/cpn|computer professionals/i,"CPN_CERTIFICATE"],
 [/nemsa/i,"NEMSA_CERTIFICATE"],
 [/iso certification/i,"ISO_CERTIFICATION"],
 [/naddc approval/i,"NADDC_APPROVAL"],
 [/similar (projects?|experience)|relevant experience/i,"EXPERIENCE"],
 [/key personnel|professional registration/i,"KEY_PERSONNEL"],
 [/language and signature/i,"LANGUAGE_SIGNATURE"],
 [/lot (bidding )?limit/i,"LOT_LIMIT"],
];
...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  const body=await req.json().catch(()=>({})); const tenderId=body?.tender_id;
  ...
  const replace=async(r:Req)=>{
   ...
   if(!rt){status="manual_review";reason="Requirement is not safely classifiable as a Vault evidence type."}
    ...
    if(!missing.length){status="matched";reason=`Audited accounts cover ${target.join(", ")}.`}
    else if(pool.length){status="manual_review";reason=`Audited accounts are incomplete; missing year(s): ${missing.join(", ")}.`}
    else{status="missing";reason="No audited financial statements were found with deterministic year metadata."}
   } else if(STATUTORY.has(rt)){
    const valid=pool.filter(statValid); best=newest(valid.length?valid:pool);
    if(!pool.length){status="missing";reason="No matching statutory document was found."}
    else if(!valid.length){status="expired";reason="The matching statutory document set has no currently valid document."}
    else{status="matched";reason="Strong canonical document type and current-year validity metadata match."}
   } else if(pool.length){
    best=newest(pool); const y=docYear(best), e=expiry(best);
    if((r.requirement_text??"").match(/20\d{2}/)&&!y){status="manual_review";reason="Candidate type matches but required year evidence is missing."}
    else if(e&&e<today()){status="expired";reason="The latest matching document is expired."}
    else{status="matched";reason="Canonical document type match using the latest active Vault version."}
   } else {status="missing";reason="No active Company Vault document matches the requirement type."}
   const confidence=status==="matched"?.99:status==="missing"||status==="expired"?0:.65;
   const explanation=best?`${reason} Candidate: ${best.document_name??best.original_filename??best.id}.`:reason;
  ...
  const total=matches.length,matched=matches.filter(x=>x.status==="matched").length,review=matches.filter(x=>x.status==="manual_review").length,missing=matches.filter(x=>x.status==="missing").length,expiredCount=matches.filter(x=>x.status==="expired").length;
  const pct=total?Math.round(matched/total*100):0;
  const matchingStatus=(missing||expiredCount||review)?"MATCHING_REVIEW":"MATCHED";
  const {error:tenderUpdateError}=await db.from("tenders").update({compliance_percentage:pct,matching_status:matchingStatus}).eq("id",t.id).eq("organization_id",t.organization_id).eq("company_id",t.company_id); if(tenderUpdateError)throw tenderUpdateError;
