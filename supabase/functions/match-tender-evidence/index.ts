...
const STOP_WORDS = new Set([
  "a", "an", "and", "are", "as", "at", "be", "by", "for", "from", "in", "is", "of", "on", "or", "that", "the", "this", "to", "with", "must", "shall", "provide", "submit", "submission", "valid", "required", "requirement",
]);
...
const tokens = (value: string) =>
  [...new Set(normalize(value).split(" ").filter((token) => token.length >= 3 && !STOP_WORDS.has(token)))];
...
const adminClient = (url: string, serviceKey: string) => {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (membershipError) return json({ error: "Could not resolve your organization." }, 500);
    ...
    const { data: requirements, error: requirementsError } = await admin
      ...
      .select("id, category, requirement_name, requirement_text, status, matched_document_id, explanation, display_order")
    ...
    const { data: documents, error: documentsError } = await admin
      ...
      .select("id, organization_id, document_name, original_filename, document_type, category, notes, storage_path, mime_type, analysis_status, document_status, deleted_at")
      .eq("organization_id", tender.organization_id)
      .is("deleted_at", null)
      .order("created_at", { ascending: false });
    if (documentsError) return json({ error: "Could not load Company Vault documents." }, 500);
    ...
    for (const requirement of requirements) {
      ...
      const ranked = (documents ?? [])
        .map((document) => ({ document, score: candidateScore(requirementText, document) }))
        .filter(({ document }) => typeof document.storage_path === "string" && document.storage_path.trim() !== "")
      ...
      if (ranked.length === 0 || ranked.every((item) => item.score === 0)) {
        ...
        const { error } = await admin.from("tender_requirements").update({
          ...
          explanation,
        }).eq("id", requirement.id).eq("tender_id", tender.id).eq("organization_id", tender.organization_id);
        if (error) return json({ error: "Could not save a requirement result." }, 500);
      ...
      for (const { document } of ranked) {
        ...
        if (mime && mime !== "application/pdf" && !path.toLowerCase().endsWith(".pdf")) {
      ...
      if (candidatePayload.length === 0) {
        const explanation = extractionIssue
          ? "A potentially relevant Vault document exists, but its contents could not be reliably read. Manual review is required."
          : "No readable evidence document was available for this requirement.";
        const status = extractionIssue ? "needs_review" : "missing";
        const { error } = await admin.from("tender_requirements").update({
          status,
          matched_document_id: null,
          explanation,
      ...
      const evaluatorPrompt = `You are a conservative procurement evidence reviewer.
Evaluate whether the supplied Company Vault document content actually satisfies the tender requirement.
Do not infer facts that are not present in the document text.
A document title or filename alone is never sufficient proof.
Return ONLY JSON:
{
...
Rules:
- found only when the document text clearly establishes the required evidence.
- needs_review when a relevant document exists but validity, period, identity, completeness, or another material condition cannot be established from the text.
- missing when the supplied candidates do not provide credible evidence.
- For found, evidence_snippet MUST be an exact short phrase copied from the selected document text (at least 12 characters).
- Never select a document whose text does not support the requirement.
- Keep explanation short and factual.`;
      ...
      const aiResponse = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            { role: "user", content: JSON.stringify({ requirement: requirementText, candidates: candidatePayload }) },
          ],
        }),
      });
      ...
      if (!aiResponse.ok) {
        ...
        if (error) return json({ error: "Could not save a requirement result." }, 500);
        results.push({ requirement_id: requirement.id, status: "needs_review", matched_document_id: null, explanation });
        continue;
      }
      ...
      try {
        evaluation = JSON.parse(typeof content === "string" ? content.replace(/^```json|```$/g, "").trim() : "");
      } catch {
        evaluation = { decision: "needs_review", document_id: null, evidence_snippet: null, explanation: "Evidence evaluation returned an unreadable result. Manual review is required." };
      }
      ...
      // Hard safety gate: an AI 'found' result is accepted only when its selected document
      // is one of our same-org candidates and its claimed evidence is literally present.
      if (decision === "found" && (!selectedCandidate || !snippet || !containsEvidenceSnippet(String(selectedCandidate.text), snippet))) {
        decision = "needs_review";
      }

      const explanation = typeof evaluation.explanation === "string" && evaluation.explanation.trim()
        ? evaluation.explanation.trim().slice(0, 500)
        : decision === "found"
          ? "The document contains clear evidence supporting this requirement."
          : decision === "needs_review"
            ? "A potentially relevant document exists, but the evidence is not sufficient for an automatic match."
            : "No credible evidence was found in the shortlisted Vault documents.";
    ...
    return json({ error: "Unexpected error during evidence matching." }, 500);
  }
});
